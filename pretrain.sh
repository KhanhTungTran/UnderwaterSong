...
for m in 0.8; do python -m torch.distributed.launch --nproc_per_node=2 main_pretrain.py --batch_size 32 --norm_pix_loss True --model mae_vit_large_patch16 --mask_ratio $m --epochs 33 --warmup_epochs 3 --save_every_epoch 4 --blr 8e-4 --weight_decay 0.0001 --dataset crs_indo --data_train /mnt/data/tungtran/AudioMAE/dataset/coral_sound_indo/train.json --label_csv /mnt/data/tungtran/AudioMAE/dataset/coral_sound_indo/class_labels_indices.csv --roll_mag_aug True --decoder_mode 1 --output_dir output_dir_indo_mask${m}_large --log_dir output_dir_indo_mask${m}_large --distributed True --world_size 2 --num_workers 8; done
