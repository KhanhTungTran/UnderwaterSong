...
df = pd.read_csv("plots/CRS_result.csv")
...
result_columns = ['indo location train 15 min', 'Indo location clf 30 min train set', 'indo location train 1h']
# result_columns = ['indo health train 15 min', 'indo health v2', 'indo health train 1h']
# result_columns = ['coral chorus location train 15 min', 'Coral chorus location clf 30 min train set', 'coral chorus location train 1h']

df[df['Freeze pretrained model in finetuning'] != (not linear_probing)]

# Define the specific x-axis points as text
x_points = ['15 min', '30 min', '60 min']
...
for model_name, display_name in model_name_dct.items():
    ...
    plt.plot(x_points, y_values, marker='o', linestyle='-', label=display_name)


# Set labels and title
plt.xlabel('Train size')
plt.ylabel('Accuracy')
plt.title(f'Performance on {task_name} with different training set size.')
